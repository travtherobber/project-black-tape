...
function renderContactList() {
    ...
    state.conversations.forEach(convo => {
        ...
        card.innerHTML = `
            <span class="contact-name">${displayName.toUpperCase()}</span>
            <span class="contact-meta">${convo.count} MESSAGES // READY</span>
        `;
...
function renderMessages(messages, targetTimestamp = null) {
    ...
    sortedMessages.forEach(msg => {
        ...
        block.innerHTML = `
            ...
            <span class="timestamp">${localTs.replace(' ', ' | ')}</span>
        `;
...
async function searchMessages(query) {
    ...
    try {
        ...
        data.results.forEach(res => {
            ...
            card.innerHTML = `
                <span class="contact-name">${res.convoId.toUpperCase()}</span>
...
function showMetadata(msg, localTs) {
    ...
    overlay.innerHTML = `
        <div class="metadata-card">
            ...
            <div class="card-body">
                <div class="intel-group"><label>Timestamp (Local)</label><span>${localTs}</span></div>
                <div class="intel-group"><label>Cluster_ID</label><span>${state.activeConversation}</span></div>
                <div class="intel-group"><label>Signal_Origin</label><span>${msg.IsSender ? "SYSTEM_OWNER" : "REMOTE_ENTITY"}</span></div>
